  ...
  return (
    <header className="flex flex-col gap-4 border-b border-[var(--border-hairline)] pb-6 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onReset}
          className="flex items-center gap-1.5 rounded-lg border border-[var(--border-hairline)] px-3 py-2 text-xs font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-2)]"
        >
          <RotateCcw size={14} />
          Reset to defaults
        </button>
        <button
          onClick={onAddCourse}
          className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold text-white shadow-lg shadow-blue-900/20 transition-transform hover:scale-[1.03] active:scale-[0.98]"
          style={{ background: 'linear-gradient(135deg, var(--accent-blue), var(--accent-aqua))' }}
        >
          <Plus size={14} />
          Add Course
